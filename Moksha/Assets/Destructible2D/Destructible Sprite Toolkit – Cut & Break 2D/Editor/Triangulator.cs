...
namespace CoreBit.DestructibleSprite
{
    public class Triangulator
    {
        ...
        public int[] Triangulate()
        {
            ...
            for (int v = nv - 1; nv > 2;)
            {
                if ((count--) <= 0 || iterations++ > 5000)
                {
                    Debug.LogWarning("Triangulator: Too many iterations. Polygon might be malformed.");
                    break;
                }
            ...
            // Disable reverse to keep winding order consistent
            // indices.Reverse(); 
            return indices.ToArray();
        }
